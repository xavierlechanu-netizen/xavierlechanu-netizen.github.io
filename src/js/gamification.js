...
window.Gamification = {
    ...
    awardXP: function(baseXP, context) {
        ...
        const drivingScore = window.DrivingScore ? window.DrivingScore.currentScore : 100;
        if (drivingScore >= 80) {
            totalXp += 50;
        ...
        const ghostMode = window.RadarSocial ? window.RadarSocial.isGhostMode : true;
        if (!ghostMode) {
            totalXp += 20;
        ...
        const blackboxConnected = document.getElementById('bb-batt') && document.getElementById('bb-batt').textContent !== '-- mV';
