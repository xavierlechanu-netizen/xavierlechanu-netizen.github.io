...
overlay.innerHTML = `
            <div style="border:4px double #000; padding:30px; max-width:800px; margin:auto; position:relative; overflow: hidden; background: #fff;">
                ...
                <button data-action="window.print()" style="margin-top:40px; width:100%; padding:15px; background:#000; color:#fff; border:none; cursor:pointer; font-weight:bold; position: relative; z-index: 1;">IMPRIMER / EXPORTER (PDF)</button>
                <button data-action="document.getElementById('cert-overlay').remove()" style="margin-top:10px; width:100%; padding:10px; background:transparent; color:#888; border:none; cursor:pointer; font-size:0.8rem; position: relative; z-index: 1;">RETOUR À L'APPLICATION</button>
            </div>
        `;
...
overlay.innerHTML = `
            <div style="border:4px solid ${borderColor}; border-radius: 10px; padding:40px; max-width:800px; margin:auto; position:relative; overflow: hidden; background: #fff;">
                ...
                <div style="margin-top:40px; display:flex; gap:10px;" class="no-print">
                    <button data-action="window.print()" style="flex:1; padding:15px; background:${borderColor}; color:#fff; border:none; border-radius:30px; cursor:pointer; font-weight:bold; font-size:1.1rem; box-shadow: 0 5px 15px rgba(0,0,0,0.2);">IMPRIMER / SAUVEGARDER PDF</button>
                    <button data-action="document.getElementById('cert-batt-overlay').remove()" style="padding:15px 30px; background:#f1f1f1; color:#333; border:none; border-radius:30px; cursor:pointer; font-weight:bold;">FERMER</button>
                </div>
            </div>
  ...
  purchaseBatteryCert: async function (tier) {
    ...
    // eslint-disable-next-line no-restricted-syntax
loader.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin" style="font-size:4rem; color:#00d2ff; margin-bottom:20px;"></i><h2 style="margin:0;">Paiement Sécurisé</h2><p id="revolut-loader-text" style="color:#aaa;">Initialisation Revolut Pay...</p>';
