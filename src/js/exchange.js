...
container.innerHTML = `
    ...
    this.filteredListings.forEach((listing) => {
      ...
      const imageContent = listing.photoUrl
        ? `<img src="${listing.photoUrl}" alt="Photo annonce" style="width:100%; height:100%; object-fit:cover; border-radius:10px;" onerror="// eslint-disable-next-line no-restricted-syntax
this.parentElement.innerHTML = '<i class=\\'fa-solid ${icon}\\'></i>'">`
      ...
      html += `
        <div class="product-card" style="position:relative; ${isReserved ? 'opacity:0.75;' : ''}">
          ...
          ${
            isOwner
              ? `<button class="buy-btn" style="background:linear-gradient(135deg, #ff4d4d, #cc0000);" data-action="ExchangeMarket.deleteListing('${listing.id}')"><i class="fa-solid fa-trash-can"></i> Supprimer</button>`
              : isReserved
              ...
              : `<div style="display:flex; gap:8px;">
                  <button class="buy-btn" style="flex:1; background:linear-gradient(135deg, #00f2ff, #0077ff);" data-action="ExchangeMarket.reserveListing('${listing.id}', ${listing.price}, '${listing.priceType}')">
                    ${isBvc ? '<i class="fa-solid fa-cart-shopping"></i> Réserver' : '<i class="fa-solid fa-handshake"></i> Acheter'}
                  </button>
                  <button class="buy-btn" style="width:42px; padding:0; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2);" data-action="ExchangeMarket.contactSeller('${listing.id}', '${listing.seller}')" title="Contacter">
...
form.innerHTML = `
      <div style="width:100%; max-width:480px; background:rgba(20,25,40,0.8); border:1px solid rgba(0,242,255,0.3); border-radius:20px; padding:25px; box-shadow:0 0 30px rgba(0,242,255,0.2); position:relative; max-height:90vh; overflow-y:auto;">
        <button data-action="ExchangeMarket.closePublishForm()" style="position:absolute; top:20px; right:20px; background:none; border:none; color:#889; font-size:1.5rem; cursor:pointer; transition:color 0.2s;" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='#889'">
