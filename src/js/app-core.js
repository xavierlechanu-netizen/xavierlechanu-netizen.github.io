...
hardLock.innerHTML = `
            ...
            <button data-action="window.repairGps()" style="width:100%; padding:15px; background:#ffb703; color:black; border:none; border-radius:30px; font-weight:bold; font-size:1.1rem; margin-bottom:15px; box-shadow:0 0 15px rgba(255, 183, 3, 0.5);">
                AUTORISER LE GPS
            </button>
  ...
  const repairBtn = `<button data-action="window.retryGps()" style="margin-top:10px; padding:8px 20px; background:#ffb703; color:#000; border:none; border-radius:20px; font-weight:bold; font-size:0.85rem; cursor:pointer;">🔄 Réessayer</button>`;
...
modal.innerHTML = `
            <div style="background:#1a1a1a; padding:25px; border-radius:15px; border:2px solid #ffb703; max-width:400px; width:100%; color:white;">
                ...
                ${instructions}
                <button data-action="this.parentElement.parentElement.remove()" style="width:100%; margin-top:20px; padding:12px; background:#ffb703; color:black; border:none; border-radius:30px; font-weight:bold; font-size:1rem;">J'AI COMPRIS</button>
            </div>
        `;
    document.body.appendChild(modal);
  }
...
toast.innerHTML = `
        ...
        <div style="display:flex; gap:10px;">
            <button data-action="confirmHazard(${index}, true)">✅ Oui</button>
            <button data-action="confirmHazard(${index}, false)">&#10060; Non</button>
        </div>
    `;
...
// --- REGIONAL & VOICE ENGINE (ORACLE v110.00.00) ---
window.OracleEngine = {
...
// --- Action Registry (ESM) ---
registerAction('updateUILabels', updateUILabels);
registerAction('checkTrialExpiration', checkTrialExpiration);
registerAction('requestWakeLock', requestWakeLock);
registerAction('hasLocationConsent', hasLocationConsent);
registerAction('checkLegalConsent', checkLegalConsent);
registerAction('showGpsBanner', showGpsBanner);
registerAction('hideGpsBanner', hideGpsBanner);
registerAction('startGeolocation', startGeolocation);
registerAction('activateLowAccuracyFallback', activateLowAccuracyFallback);
registerAction('getSmoothedSpeed', getSmoothedSpeed);
registerAction('updatePosition', updatePosition);
registerAction('getDistance', getDistance);
registerAction('checkHazardProximity', checkHazardProximity);
registerAction('showHazardConfirmation', showHazardConfirmation);
registerAction('vibrate', vibrate);
registerAction('speak', speak);
registerAction('checkNightMode', checkNightMode);
